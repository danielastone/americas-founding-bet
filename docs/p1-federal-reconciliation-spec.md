...
- for `analysis_ready` rows, a `verification_record_id` that joins to retained verification evidence and identifies the verifier and verification date;
- if a purported independent check is unattributed, retain the calculation at `validated` or lower and label any period result provisional;
